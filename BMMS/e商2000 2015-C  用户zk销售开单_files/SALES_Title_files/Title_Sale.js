...
function MyPrint()
{
	...
	if (document.Title.SalesID.value=='NULL'||document.Title.SalesID.length==0)
		alert('请选择保存过的单据！');
	else{
		//parent.View.location.replace('SALES_Print.asp?ID=Sales&SerialNo='+document.Title.SalesID.value+'&time='+new Date().valueOf());
		printURL('SALES_Print.asp?ID=Sales&SerialNo='+document.Title.SalesID.value+'&time='+new Date().valueOf());
		}
}
function MyKeyDown(){var f=window.event.srcElement.form;if(f==testundefined)return;
l=f.elements.length;k=window.event.keyCode;e=window.event.srcElement;n=e.name;
if(k==13&&n=='Custom'){getcsulist('Custom',5,e.value);window.event.returnValue=false;return;}
...
document.Title.Custom.focus();
if (parent.Run.rtTestUser('Sale',128,0)){document.Title.discount.value='1.00';document.Title.discount.readOnly=1;}
