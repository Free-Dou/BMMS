...
public class MySqlIO {
	...
	public boolean verifyPwd(String userName, String pwd) {
		...
		logger.info("[MySqlIO.java:verifyPwd] " + sql + "    params: username = '" + userName + "'  pwd = '" + pwd
				+ "'  salt = '20160518';");
	...
	public ArrayList<ProjectQunatity> getAllProjectQunatityInfo() {
		...
		try {
			...
			while (rs.next()) {
				...
				String contractContent = rs.getString("contractContent");
				String contractNumber = rs.getString("contractNumber");
				String partyA = rs.getString("partyA");
				String constructLoca = rs.getString("constructLoca");
				String constructDate = rs.getString("constructDate");
				String water = rs.getString("water");
				Float waterSelfProduct = rs.getFloat("waterSelfProduct"); /* 3位小数 */
				Float waterBuy = rs.getFloat("waterBuy"); /* 3位小数 */
				Float waterPrice = rs.getFloat("waterPrice"); /* 2位小数 */
				String blackMaterial = rs.getString("blackMaterial");
				Float blackMaterialSelfProduct = rs.getFloat("blackMaterialSelfProduct"); /* 3位小数 */
				Float blackMaterialBuy = rs.getFloat("blackMaterialBuy"); /* 3位小数 */
				Float blackMaterialPrice = rs.getFloat("blackMaterialPrice"); /* 2位小数 */
				Float blackMaterialSell = rs.getFloat("blackMaterialSell"); /* 2位小数 */
				String remark = rs.getString("remark");
				...
				projectQunatityObject = new ProjectQunatity(projectName, budget, paid, contractContent, contractNumber,
						partyA, constructLoca, constructDate, water, waterSelfProduct, waterBuy, waterPrice,
						blackMaterial, blackMaterialSelfProduct, blackMaterialBuy, blackMaterialPrice,
	...
	public ProjectQunatity getOneProjectQunatityInfoById(String projectId) {
		...
		try {
			...
			logger.info("[MySqlIO.java:getOneProjectQunatityInfoById] Get one ProjectQunatity Info Success!!!");
		} catch (SQLException e) {
			logger.error("[MySqlIO.java:getOneProjectQunatityInfoById] Get ProjectQunatity Info Failed!!!");
			logger.error("Error Message : " + e.getMessage());
			e.printStackTrace();
		} finally {
			/* 关闭资源 */
	...
	public ArrayList<PersionMessage> getPersionMessageInfo(String sql, String params[]) {
		...
		try {
			...
			while (rs.next()) {
				...
				persionMessageObject = new PersionMessage(orderid, mname, carNum, mpspec, number, stockLoca, price,
						totalPrice, username, createTime, relationName, operation, approval, remark, orderRemark);
		...
		try {
			...
			while (rs.next()) {
				...
				if (null == lastOrderID) { /* 如果是第一条数据 */
					...
					logger.info(
							"[MySqlIO.java:getAllWareHousingOrderInfo]  Get WareHousing order's product : " + mpSpec);
					lastOrderID = orderID; /* 更新orderID */
				} else if (orderID.equals(lastOrderID)) { /* 如果是同一个订单的数据 */
					wareHousingOrderObject.AddWareHousingProduct(mpSpec, pName, pCount, pPrice, pTotalPrice, pRemark);
					logger.info(
	...
	public void delInfoFromDB(String sql, String[] parameters) {
		...
		logger.info(
				"[MySqlIO.java:delInfoFromDB] del info from DB: " + sql + "  params: " + Arrays.toString(parameters));
	...
	// }
	//
	// webPageObject = new WebPageObejct(url, abstract_info, primary_path,
	// mainbody_path);
	...
	// } finally {
	// // 关闭资源
	// sqlHelper.closeDB(rs, sqlHelper.getPreparedStatement(),
	...
	// try {
	// while (rs.next()) {
	// logger.info("url: " + webpage_url + " has already exist in " +
	// table_name);
	// result = true;
	// }
	// } catch (SQLException e) {
	...
	// }
	//
	//// public boolean updateById(String table_name, WebPageObejct
	// webPageObject) {
	////
	...
	//// webPageObject.getWebpage_primary_path(),
	...
	//// }
	////
	//// public boolean deleteBywebpage_url(String table_name, String
	// webpage_url) {
	////
	...
	// webPageObject.getWebpage_abstract_info(),
	// webPageObject.getWebpage_primary_path(),
	// webPageObject.getWebpage_mainbody_path() };
	//
	// logger.info("sql: " + sql);
	...
	// logger.error(e.getMessage());
	// } finally {
	// sqlHelper.closeDB(null, sqlHelper.getPreparedStatement(),
	// sqlHelper.getConnection());
	// }
	//
	// return result;
