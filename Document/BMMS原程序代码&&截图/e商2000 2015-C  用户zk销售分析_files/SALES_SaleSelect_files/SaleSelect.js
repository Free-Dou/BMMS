...
for (i=0;i<obj.length;i++){
	...
	else if (obj.elements[i].type=='select-one')value = obj.elements[i].selectedIndex;
...
document.senddata.action = 'SALES_SaleSelect.ASP?fun=create&FunctionName='+escape(document.search.funtext.value)+'&time='+t;
...
document.senddata.action='SALES_SaleSelect.ASP?fun=delete&FunctionName='+escape(document.search.funlist.options[document.search.funlist.selectedIndex].text)+'&time='+t;
document.senddata.method = "POST"
document.senddata.submit();
}
function fill(str){
...
if (document.search.funlist.selectedIndex == -1) return;
t = new Date().valueOf();
document.location.replace('SALES_SaleSelect.asp?FunctionName='+document.search.funlist.options[document.search.funlist.selectedIndex].text+'&time='+t);
}
...
if (document.search.newface.checked) str ='SALES_SaleViewEx.asp?search=1';
else str ='SALES_SaleView.asp?search=1';
 
obj = document.search;

str+='&fos='+obj.fos.options[obj.fos.selectedIndex].value;
str+='&mt='+obj.mt.options[obj.mt.selectedIndex].value;
...
str+='&iscustom='+(obj.customtest.checked?1:0);
str+='&customtext='+escape(obj.customtext.value);
...
str+='&ischeckprice='+(obj.checkprice.checked?1:0);
...
dynload.location.replace('DYNLoad/Load_CSU.asp?cb=e&csu=1&time='+new Date().valueOf());
}